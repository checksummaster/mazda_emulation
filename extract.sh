...
for f in *_reinstall.up; do
	...
	then
		...
		sed '/<body>/a\
		<script src="../../../boot.js" type="text/javascript"></script><script src="apps/custom/additionalApps.js" type="text/javascript"></script>' $(basename $f _reinstall.up)/jci/gui/index.html > tmp.txt
		mv tmp.txt $(basename $f _reinstall.up)/jci/gui/index.html	
		ln -sf ${BASE}/apps $(basename $f _reinstall.up)/jci/gui/apps/custom
	fi
done
